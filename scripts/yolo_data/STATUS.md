...
## 当前进度（2026-09-07 晚）
- 有效样本：**11 个原始 + 33 个离线增强 = 44 个**（共 29 个真实标注目标）
- 最新模型：`yolo_train/runs/captcha_yolov8n-4/weights/best.pt`（已部署服务器 ~/sgcc/yolo_model/best.pt）
- 验证指标（44 样本数据集，val 16 张/41 实例）：
  - **mAP50 = 0.841**，Precision 0.80，Recall 0.80
  - 全样本命中率：conf=0.25 → 97%（28/29），conf=0.1 → **100%（29/29）**（IoU≥0.4）
- **新增监听模式**：服务器 `captcha_watch.py` —— 用户正常登录时旁观记录验证码点击，零干扰。今晚首次实战：3 个点击全部有效入训练集（sample_010），且新模型 3/3 命中该样本 ✅
...
| **captcha_yolov8n-4（09-07 晚）** | **44 样本（+增强）** | **mAP50 0.841，命中率 100%** |
